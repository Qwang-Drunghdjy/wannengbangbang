...
- 点击行为：
  - **发布寻物**（🔍）→ 跳转 `/publish?type=seek`（寻物启事：我丢了东西，找它）
  - **发布拾物**（📦）→ 跳转 `/publish?type=claim`（拾物招领：我捡到东西，还给它）
...
- 导航项：
  - 🏠 首页 → `/`
  - 🔔 消息 → `/messages`（有未读时显示红色角标，数量来自全局状态）
  - 👤 我的 → `/profile`
...
- 点击后触发文件选择器（仅图片），选择后显示缩略图（占位）；可重新选择或删除。
- **必填规则**：`claim`（拾物招领）**必填**（后端 `image_url` 非空约束）；`seek`（寻物启事）**可选**。
- 演示阶段：上传后弹出 `alert("已上传")` 并显示缩略图（占位）。
...
- 头像：圆形，直径 `64px`，显示 `"👤"` 占位（有头像则显示头像 URL）。
